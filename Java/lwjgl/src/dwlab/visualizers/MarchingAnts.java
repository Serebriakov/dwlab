...
public class MarchingAnts extends Visualizer {
	...
	private static final Vector serviceVector11 = new Vector();
	private static final Vector serviceVector12 = new Vector();
	private static final Vector serviceVector21 = new Vector();
	private static final Vector serviceVector22 = new Vector();
	...
	public static void drawMARect( double x, double y, double width, double height, int pos ) {
		...
		lineImage.draw( 0, Math.floor( x - width2 ), Math.floor( y - height2), Math.floor( width ), 1d, pos, 0, (int) width + pos, 1, 0d, Color.white );
		pos += width;
		lineImage.draw( 0, Math.floor( x + width2 ), Math.floor( y - height2), 1d, Math.floor( height ), 0, pos, 1, (int) height + pos, 0d, Color.white );
		pos += height;
		lineImage.draw( 0, Math.floor( x + width2 ), Math.floor( y + height2), -Math.floor( width ), 1d, pos, 0, (int) width + pos, 1, 0d, Color.white );
		pos += width;
		lineImage.draw( 0, Math.floor( x - width2 ), Math.floor( y + height2), 1d, -Math.floor( height ), 0, pos, 1, (int) height + pos, 0d, Color.white );
	...
	public static void drawMALine( double x1, double y1, double x2, double y2, int pos ) {
		...
		lineImage.draw( 0, 0.5d * ( x1 + x2 ), 0.5d * ( y1 + y2 ), length, 1d, pos, 0, (int) length + pos, 1, 0d, Color.white );
	}
}